...
$(function($) {
    ...
    $(document).ajaxError(function (event, XMLHttpRequest, textStatus, errorThrow) {
        if(XMLHttpRequest.status == 400) {
            ...
            toastr.error(cadena.message, "Mensaje Servidor");   
        }
        else {
...
$(document).ready(function () {
    $.ajax({
        type: 'get',
        url: '/DatosUsuarioActivo',                   
        success: function (response) {                         
            $(".userNAME").text(response.data.nombre);                
            if(response.data.perfil_id==0){
                $(".perfilNAME").text("Administrador"); 
            }   
            if(response.data.perfil_id==1){
                $(".perfilNAME").text("Usuario"); 
            }             
        },
    });
});